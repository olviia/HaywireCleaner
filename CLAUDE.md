...
## Collaboration style (how to work with this developer)

- **Code policy**: never use Edit or Write on code files (.cs or otherwise). Explain
...
- **Reasoning style**: work through worked examples, and trace decisions through the
...
- **Conversational tone**: concise and direct. Ask clarifying questions before
  ...
  Don't pad responses with recap/summary of what was just discussed — the developer
  is tracking the conversation themselves.
- **Process discipline**: stick to the agreed step-by-step process (see Process and
  Status below). Don't jump ahead into later steps (architecture, vertical slice,
  full systems) while an earlier step is still open, even if it would be more fun
  or feel productive — resist scope creep.

## Process (agreed approach)
...
2. **Decide which RPG systems this game actually needs, deliberately** — using MDA
   ...
   experience rather than inherited genre convention. *Currently in progress.*
...
Genre and core design are locked. Currently working through Step 2: tracing
candidate RPG systems (inventory, stats, appearance, quests) through MDA to decide
which are foundational to this game's experience vs. inherited convention. That
list will directly inform both the vertical slice's scope and the architecture's
extension points.